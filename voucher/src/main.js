import './style.css';
import { trip as sampleTrip, seedExpenses, categoryLabels } from './data.js';
...
const storageKey = 'ouranos:voucher:v2';
const initial = () => ({ trip: structuredClone(sampleTrip), source: 'sample', intakeComplete: true, expenses: structuredClone(seedExpenses), resolutions: {}, audit: [], pending: null });
let state;
try { state = { ...initial(), ...JSON.parse(sessionStorage.getItem(storageKey) || '{}') }; }
catch { state = initial(); }
let trip = state.trip;
  ...
  app.innerHTML = `
    ...
    <main class="shell">
      ...
      <section class="trip-head">
        <div><div class="eyebrow">${state.source === 'sample' ? 'SAMPLE TRIP' : 'APPROVED TRIP'} · ${escapeHtml(trip.authorizationId)}</div><h1>Finish your ${escapeHtml(trip.destination)} trip</h1><p class="subhead">${escapeHtml(trip.origin)} → ${escapeHtml(trip.destination)} · ${formatDate(trip.startDate)}–${formatDate(trip.endDate)}</p></div>
        <div class="trip-actions"><div class="trip-status"><span class="status-dot"></span> Authorization approved</div><button class="reset-link" data-action="load-authorization">Use another authorization</button>${state.source === 'sample' ? '' : '<button class="reset-link" data-action="load-sample">Return to sample trip</button>'}</div>
      </section>
      ...
      <div class="content-grid">
        ...
        <aside class="side-column"><section class="summary-card"><div class="eyebrow">VOUCHER SUMMARY</div><h2>${!started ? 'Not started' : count || !readyForReview ? 'Almost there' : 'Ready for review'}</h2><div class="summary-line"><span>Authorized</span><strong>${currency(result.totals.authorized)}</strong></div><div class="summary-line"><span>Actual expenses</span><strong>${currency(result.totals.actual)}</strong></div><div class="summary-divider"></div><div class="summary-line"><span>Paid with GTCC</span><strong>${currency(result.totals.gtcc)}</strong></div><div class="summary-line"><span>Paid personally</span><strong>${currency(result.totals.traveler)}</strong></div><div class="summary-divider"></div><div class="summary-line"><span>Required receipts</span><strong>${!started ? 'Not started' : result.issues.some(x => x.code === 'receipt_missing') ? 'Incomplete' : 'Complete'}</strong></div><div class="summary-line"><span>Open exceptions</span><strong>${count}</strong></div><button class="button button-primary export-button" data-action="export" ${!readyForReview ? 'disabled title="Finish expense intake and resolve all exceptions first"' : ''}>Download review package ↓</button><p class="summary-note">Amounts are organized for review. Final reimbursement and policy decisions happen in the approved workflow.</p></section>
          ...
          ${state.source === 'sample' ? '<button class="reset-link" data-action="reset">Reset sample trip</button>' : ''}
        </aside>
      </div>
...
function expenseForm() {
  ...
  const options = trip.authorizedItems.map(item => `<option value="${item.id}" ${expense.authorizationItemId === item.id ? 'selected' : ''}>${escapeHtml(item.label)} · ${currency(item.amount)}</option>`).join('');
  ...
  return `<form id="expense-form" class="expense-form">
    ...
    ${extracted ? `<div class="match-box"><strong>Suggested placement</strong><p id="match-reason">${escapeHtml(suggestion?.reason || 'Choose the approved item before saving.')}</p><label>Match receipt to<select name="existingExpenseId"><option value="">Create a new expense</option>${existingOptions}</select></label></div>` : ''}
    <details class="receipt-details" ${!extracted || needsDetails ? 'open' : ''}><summary>${extracted ? 'Review scanned details' : 'Expense details'}</summary><div class="form-grid">
      ...
      <label>Authorized item<select name="authorizationItemId"><option value="">No match</option>${options}</select></label>
...
function updatePurposeSuggestion(event) {
  ...
  form.elements.authorizationItemId.value = suggestion.authorizationItemId;
...
function handleAction(event) {
  ...
  if (action === 'load-sample' && confirm('Leave this voucher and return to the sample trip?')) { state = initial(); trip = state.trip; save(); render(); toast('Sample trip loaded.'); }
  ...
  if (action === 'reset' && confirm('Reset this sample trip and clear your changes?')) { state = initial(); trip = state.trip; save(); render(); toast('Sample trip reset.'); }
}
...
function saveExpense(event) {
  ...
  const expense = {
    ...old, id: old.id || crypto.randomUUID(), tripId: trip.id,
  ...
  if (state.source !== 'sample') state.intakeComplete = false;
...
function applyAuthorization(input, source = 'imported') {
  ...
  state = { trip: approved, source, intakeComplete: false, expenses: [], resolutions: {}, audit: [], pending: null };
  trip = approved;
  log(`Loaded approved authorization ${approved.authorizationId}`);
  save(); render(); toast('Approved trip loaded. Scan a receipt to begin.');
}
...
async function loadAuthorizationFromRoute() {
  ...
  try {
    ...
    if (!response.ok) throw new Error(`Authorization ${id} is not available yet. Use an approved authorization JSON file.`);
