...
export const expenseInput = z.object({tripId:uuid, merchant:text(200), incurredOn:dateOnly, amountMinor:z.number().int().nonnegative().max(Number.MAX_SAFE_INTEGER), currency:z.string().regex(/^[A-Z]{3}$/), category:z.enum(['lodging','transport','meals','other']), description:z.string().max(2000).default(''), documentIds:z.array(uuid).max(30).default([])}).strict();
