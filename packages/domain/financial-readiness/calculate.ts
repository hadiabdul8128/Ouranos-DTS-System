...
function perPaycheck(monthly:number[],annual:number){
 const raw=monthly.map(v=>v*12/annual),result=raw.map(Math.floor);
...
export function calculateFinancialReadiness(input:FinancialProfile,asOfMonth=new Date().toISOString().slice(0,7)):FinancialCalculation{
 ...
 const monthlyTakeHomeMinor=Math.round(p.takeHomePerPaycheckMinor*annualPaychecks/12);
 const monthlyTspMinor=Math.round(p.tspPerPaycheckMinor*annualPaychecks/12);
 ...
 const emergencyTopUpMinor=Math.min(Math.max(0,monthlyUnassignedMinor),Math.max(0,p.emergency.targetMinor-p.emergency.balanceMinor-p.emergency.monthlyContributionMinor));
 const remaining=Math.max(0,monthlyUnassignedMinor-emergencyTopUpMinor);
 const goalTopUpMinor=goalGapMinor===0?0:Math.min(remaining,additionalMonthlyGoalMinor===null?Math.max(0,goalGapMinor-p.goal.monthlyContributionMinor):additionalMonthlyGoalMinor);
