...
export async function buildApp(config:PlatformConfig,options:{pool?:Pool;logger?:boolean}={}){
 ...
 app.put('/v1/organizations/:id/members',async req=>{
  ...
  return withActor(pool,req.actor.id,org,async db=>{
   ...
   await db.query('insert into ouranos.memberships(organization_id,user_id,role,active) values($1,$2,$3,$4) on conflict(organization_id,user_id) do update set role=excluded.role,active=excluded.active',[org,b.userId,b.role,b.active]);
 ...
 app.get('/v1/sync/pull',async req=>{const q=querySchema.parse(req.query);return withActor(pool,req.actor.id,q.organizationId,async db=>{
  ...
  const rows=(await db.query('select * from ouranos.change_log where organization_id=$1 and cursor>$2 order by cursor limit $3',[q.organizationId,q.cursor,q.limit+1])).rows;
 ...
 app.get('/v1/approvals/:id/revision',async req=>{const id=uuid.parse((req.params as any).id),org=uuid.parse((req.query as any).organizationId);return withActor(pool,req.actor.id,org,async db=>{const a=await loadEntity(db,'approval',id,org);return {revision:(await db.query('select * from ouranos.submission_revisions where id=$1',[a.revision_id])).rows[0],steps:(await db.query('select * from ouranos.approval_steps where request_id=$1 order by position',[id])).rows,decisions:(await db.query('select * from ouranos.approval_decisions where request_id=$1 order by created_at',[id])).rows}})});
 ...
 app.get('/v1/authorizations/:id/working',async req=>{check(config.APPROVAL_MODE==='preview','PERMISSION_DENIED','Working plans are unavailable while approvals are required',403);const id=uuid.parse((req.params as any).id),org=uuid.parse((req.query as any).organizationId);return withActor(pool,req.actor.id,org,async db=>({revision:await loadWorkingPlan(db,org,id)}))});
 app.get('/v1/vouchers/:id/package',async req=>{const id=uuid.parse((req.params as any).id),org=uuid.parse((req.query as any).organizationId);return withActor(pool,req.actor.id,org,async db=>{const v=await loadEntity(db,'voucher',id,org);return config.APPROVAL_MODE==='preview'&&['draft','changes_requested'].includes(v.status)?loadDraftVoucherPackage(db,org,id):loadVoucherPackage(db,org,id)})});
 app.get('/v1/vouchers/:id/verification',async req=>{const id=uuid.parse((req.params as any).id),org=uuid.parse((req.query as any).organizationId);return withActor(pool,req.actor.id,org,async db=>{
  await loadEntity(db,'voucher',id,org);
  const row=(await db.query(`select v.report,r.id as revision_id,r.sha256,r.snapshot,r.created_at from ouranos.voucher_verifications v
   join ouranos.submission_revisions r on r.organization_id=v.organization_id and r.id=v.revision_id
   where v.organization_id=$1 and v.voucher_id=$2 order by v.created_at desc,v.id desc limit 1`,[org,id])).rows[0];
  check(row,'NOT_FOUND','No verification has been run for this voucher',404);
  return {report:row.report,revision:{id:row.revision_id,sha256:row.sha256,snapshot:row.snapshot,createdAt:row.created_at}};
 })});
 app.get('/v1/trips/:id/audit',async req=>{const id=uuid.parse((req.params as any).id),org=uuid.parse((req.query as any).organizationId);return withActor(pool,req.actor.id,org,async db=>{await loadEntity(db,'trip',id,org);return {events:(await db.query('select id,action,actor_id,entity_id,details,created_at from ouranos.audit_events where organization_id=$1 and trip_id=$2 order by id limit 500',[org,id])).rows}})});
