...
export async function loadEntity(db:PoolClient,kind:EntityKind,id:string,organizationId:string,lock=false){
 const r=await db.query(`select * from ouranos.${TABLES[kind]} where id=$1 and organization_id=$2${lock?' for update':''}`,[id,organizationId]);
...
export async function publishChange(db:PoolClient,kind:EntityKind,row:Record<string,any>){
 ...
 await db.query('insert into ouranos.change_log(organization_id,trip_id,kind,entity_id,entity) values($1,$2,$3,$4,$5)',[e.organizationId,e.tripId||(kind==='trip'?e.id:null),kind,e.id,JSON.stringify(e)]);
 return e;
}
export async function updateStatus(db:PoolClient,kind:EntityKind,id:string,org:string,status:string){
 const r=await db.query(`update ouranos.${TABLES[kind]} set status=$3,version=version+1,updated_at=now() where id=$1 and organization_id=$2 returning *`,[id,org,status]);
