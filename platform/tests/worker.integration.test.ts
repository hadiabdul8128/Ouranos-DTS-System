...
async function fixture(priorAttempts=0){const id=crypto.randomUUID();await pool.query("insert into public.documents(id,organization_id,trip_id,data,storage_key,status,version,created_by) values($1,$2,$3,$4,$5,'uploaded',2,$6)",[id,organizationId,tripId,data,`${organizationId}/${tripId}/${id}/original`,userId]);const job=await enqueue(id,2);if(priorAttempts)await pool.query('update pgmq.q_ouranos_jobs set read_ct=$2 where msg_id=$1',[job.messageId,priorAttempts]);return job}
async function current(id:string){return (await pool.query('select status,version from public.documents where id=$1',[id])).rows[0]}
...
async function newerAttempt(id:string){
 ...
 const row=await promptly(withActor(pool,userId,organizationId,async db=>{const updated=(await db.query("update public.documents set status='uploaded',version=version+2,updated_at=now() where id=$1 and organization_id=$2 returning *",[id,organizationId])).rows[0];await publishChange(db,'document',updated);return updated}));
...
beforeAll(async()=>{
 ...
 await pool.query('insert into public.organizations(id,name,created_by) values($1,$2,$3)',[organizationId,'Worker concurrency fixtures',userId]);
 await pool.query("insert into public.memberships(organization_id,user_id,role) values($1,$2,'admin')",[organizationId,userId]);
 await pool.query('insert into public.trips(id,organization_id,traveler_id,data,created_by) values($1,$2,$3,$4,$3)',[tripId,organizationId,userId,{destination:'Fixture',departure:'2026-10-12',returnDate:'2026-10-13',purpose:'Worker concurrency test'}]);
});
afterAll(async()=>{
...
describe('worker claims, provider isolation, and queue fencing',()=>{
 it('allows ordinary workspace access while a receipt provider is waiting',async()=>{
  ...
  try{
   ...
   read=withActor(pool,userId,organizationId,async db=>(await db.query('select id,status from public.documents where id=$1',[job.id])).rows[0]);
 ...
 it('deduplicates simultaneous messages and records one extraction and completion',async()=>{
  ...
  expect((await pool.query('select * from public.extraction_runs where document_id=$1',[job.id])).rowCount).toBe(1);
  expect((await pool.query('select * from public.job_results where job_key=$1',[job.jobKey])).rowCount).toBe(1);
  expect(await current(job.id)).toEqual({status:'needs_review',version:3});
 });
 it('discards a stale successful response after a newer attempt completes',async()=>{
  ...
  expect((await pool.query('select model_version from public.extraction_runs where document_id=$1',[job.id])).rows).toEqual([{model_version:'current-model'}]);
  expect((await pool.query('select * from public.job_results where job_key=$1',[job.jobKey])).rowCount).toBe(0);
 });
 it('does not let an exhausted stale failure overwrite a newer success',async()=>{
  ...
  expect(await current(job.id)).toEqual({status:'needs_review',version:5});
  expect((await pool.query("select * from public.failed_jobs where message->>'jobKey'=$1",[job.jobKey])).rowCount).toBe(0);
  expect((await pool.query('select model_version from public.extraction_runs where document_id=$1',[job.id])).rows).toEqual([{model_version:'recovered-model'}]);
 });
 it.each(['expired','reclaimed'] as const)('does not publish after its queue lease is %s',async reason=>{
  ...
  expect((await pool.query('select * from public.extraction_runs where document_id=$1',[job.id])).rowCount).toBe(0);
  expect((await pool.query('select * from public.job_results where job_key=$1',[job.jobKey])).rowCount).toBe(0);
 ...
 it('publishes a terminal failure only for the current processing version',async()=>{
  ...
  expect((await pool.query("select error_code from public.failed_jobs where message->>'jobKey'=$1",[job.jobKey])).rows).toEqual([{error_code:'PROVIDER_HTTP_503'}]);
 });
});
