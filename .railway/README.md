...
Each service is connected to GitHub `main`, which is also the repository default branch. Watch patterns avoid rebuilding unrelated services. Railway waits for GitHub Actions to pass before automatically deploying code changes. Infrastructure changes require a separate plan/apply. The worker and scanner have no public domains.
